...
export function EducationCard({ item, index }: EducationCardProps) {
    return (
        <motion.div
            ...
            viewport={{ once: true }}
        >
            <Card className="h-full relative overflow-hidden group rounded-3xl border-none shadow-lg hover:shadow-2xl shadow-primary-400/25 hover:shadow-primary-400/50 transition-all duration-500 p-0">
                                ...
                                <div className="flex flex-col gap-2">
                                    {item.group && (
                                        <div className="flex items-center gap-3 text-muted-foreground">
                                            ...
                                            <span className="text-sm font-medium">Group: {item.group}</span>
                                        </div>
                                    )}
                                    {item.board && (
                                        <div className="flex items-center gap-3 text-muted-foreground">
                                            <IconCertificate className="h-4 w-4 text-primary shrink-0" />
                                            <span className="text-sm font-medium">Board: {item.board}</span>
                                        </div>
                                    )}
                                </div>
                            </div>
                        ...
                        {item.result && (
                            <div className="flex gap-4 pt-2">
                                <div className="text-sm">
                                    <span className="text-muted-foreground block text-xs uppercase tracking-wider font-bold mb-1">Result</span>
